    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 px-4">
        ...
        <div class="md:col-span-3 bg-base-100 p-4 rounded shadow overflow-x-auto">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
                <h2 class="text-xl font-bold">LISTADO DE GRUPOS</h2>
                <a href="{{ route('grupos.create') }}" class="btn btn-success w-full sm:w-auto">+ Nuevo grupo</a>
            </div>
            ...
            <table class="table table-zebra w-full text-sm">
                ...
                <tbody>
                    @foreach ($grupo as $gru)
                        <tr>
                            <td>{{ $gru->nombre }}</td>
                            <td>{{ $gru->programa->nombre }}</td>
                            <td>{{ $gru->sede->nombre }}</td>
                            <td class="flex flex-col sm:flex-row gap-1">
                                <a href="{{ route('grupos.edit', $gru->id) }}" class="btn btn-sm btn-warning">Editar</a>
                                <form action="{{ route('grupos.destroy', $gru->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar este docente?')">
                                    @csrf
                                    @method('DELETE')
